...
- 修复热点/Wi-Fi 切换期间自动修复过早重建 TUN 的问题：连续确认 IP、默认网关、网关 DNS 端口和国内直连稳定后才允许后台 TUN 修复，否则只等待下一轮复检。
- 手动一键修复在 TUN/节点处理后仍发现内外网同时失败时，会重绑定当前 Wi-Fi 服务、重新获取 DHCP、同步 Clash 代理并重建 TUN，模拟新建网络位置的重载效果，但不创建新的网络位置。
- 网络路径监听增加状态去重，减少 macOS 网络切换期间的重复扫描和反复修复。
